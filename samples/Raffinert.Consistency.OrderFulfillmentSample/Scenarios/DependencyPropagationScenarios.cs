...
internal static class DependencyPropagationScenarios
{
    public static void Run()
    {
        ...
        var fulfilledQuantity = model.Derived(lines).Using(matchingFulfillments).Impact(p => p.MembershipAdded(DependencySeverity.Dirty).MembershipRemoved(DependencySeverity.Invalid).ItemChanged(DependencySeverity.Invalid)).Incrementally().Compute((_, rows) => rows.Sum(x => x.Quantity)).Named("fulfilled-quantity");
        var remainingQuantity = model.Derived(lines).Using(fulfilledQuantity).Impact(p => p.SourceChanged(DependencySeverity.Dirty).SourceMemberChanged(x => x.OrderedQuantity, (oldValue, newValue) => newValue < oldValue ? DependencySeverity.Invalid : DependencySeverity.Dirty)).Compute((l, fulfilled) => l.OrderedQuantity - fulfilled).Named("remaining-quantity");
        var unitRate = model.Derived(lines).Impact(p => p.SourceChanged(DependencySeverity.Invalid)).Compute(x => x.UnitRate).Named("unit-rate");
        var allocationValidity = model.Derived(allocations).Using(x => x.OrderLine, remainingQuantity, unitRate).Impact(p => p.SourceChanged(DependencySeverity.Invalid)).Compute((allocation, remaining, rate) => allocation.ReservedQuantity <= remaining && allocation.CapturedRate == rate).Named("allocation-validity");
        var allocationInvariant = model.Invariant(allocations).Using(allocationValidity).Must((_, value) => value).Named("allocation-validity-invariant").ScheduleRepairWith(x => repairs.Add(x.Id));
        ...
        Require(runtime.Version == 0, "bootstrap should not create a mutation version"); _ = runtime.Get(allocationValidity, allocation); _ = runtime.Evaluate(allocationInvariant, allocation);
        line.OrderedQuantity = 12; runtime.Apply(Change.Property(lines, line, x => x.OrderedQuantity, 10m, 12m));
        Require(runtime.GetState(remainingQuantity, line) == DerivedValueState.Dirty, "increase should be dirty"); _ = runtime.Get(allocationValidity, allocation); _ = runtime.Evaluate(allocationInvariant, allocation);
        ...
        _ = runtime.Get(allocationValidity, allocation); _ = runtime.Evaluate(allocationInvariant, allocation); repairs.Clear(); fulfillment.Cancelled = true;
